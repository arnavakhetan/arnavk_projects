...
public class CompactPrefixTree implements Dictionary {
    ...
    public String[] suggest(String word, int numSuggestions) {
        ...
        suggest(word, root, "", suggestions, numSuggestions);
        return suggestions.toArray(new String[0]);
    }
    ...
    private boolean check(String s, Node node) {
        ...
        int diff = findIndexDifference(s, node.prefix);
        if (diff < node.prefix.length()) {
            return false;
        }
        if (diff == s.length()) {
            return node.isWord;
        }
        int index = s.charAt(diff) - 'a';
        if (index < 0 || index >= 26) {
            return false;
        }
        return check(s.substring(diff), node.children[index]);
    }
    ...
    private boolean checkPrefix(String prefix, Node node) {
        ...
        int diff = findIndexDifference(prefix, node.prefix);
        if (diff < prefix.length() && diff < node.prefix.length()) {
            return false;
        }
        if (diff == prefix.length()) {
            return true;
        }
        int index = prefix.charAt(diff) - 'a';
        if (index < 0 || index >= 26) {
            return false;
        }
        return checkPrefix(prefix.substring(diff), node.children[index]);
    }
    ...
    private void suggest(String word, Node node, String currentWord, List<String> suggestions, int numSuggestions) {
        if (node == null || suggestions.size() >= numSuggestions) {
            return;
        }
        String combinedWord = currentWord + node.prefix;
        int prefixLength = findIndexDifference(word, combinedWord);

        if (prefixLength < combinedWord.length() && prefixLength < word.length()) {
            int index = word.charAt(prefixLength) - 'a';
            if (index >= 0 && index < 26 && node.children[index] != null) {
                suggest(word, node.children[index], combinedWord, suggestions, numSuggestions);
            }
        }
        if (node.isWord && !suggestions.contains(combinedWord)) {
            suggestions.add(combinedWord);
        }
        for (Node child : node.children) {
            if (child != null) {
                suggest(word, child, combinedWord, suggestions, numSuggestions);
